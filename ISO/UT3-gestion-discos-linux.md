...
Para crear una partición en un disco, ejecutaremos el comando `fdisk /dev/sdb`. Una vez dentro de la interfaz, utilizaremos la tecla `g` para asignarle una etiqueta GPT al volumen y a continuación `n` para añadir una nueva partición en el disco sdb. <!-- Cuando el sistema nos pregunte qué tipo de partición queremos crear, extendida (`e`) o primaria (`p`), escogemos la segunda pulsando la tecla correspondiente y ejecutamos con la tecla `intro`.-->

>[!IMPORTANT]
>En este ejemplo con el comando `fdisk /dev/sdb` nos referimos al disco sdb _(Sata Disk B)_ pero en el caso de que tengamos otros, el nombre cambiará. Por defecto Linux los denominará de manera consectiva como sda, sdb, sdc, etc.
